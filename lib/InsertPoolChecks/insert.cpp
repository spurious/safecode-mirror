///
/// $Id: insert.cpp,v 1.83 2008-08-29 03:03:39 mai4 Exp $
///
...
static RegisterPass<InsertPoolChecks> ipc("safecode", "insert runtime checks");
...
#undef REG_FUNC
  ...
  bool InsertPoolChecks::doInitialization(Module & M) {
    addCheckProto(M);
  ...
  bool InsertPoolChecks::runOnFunction(Function &F) {
    addPoolChecks(F);

    // Add stack registrations
    registerStackObjects (F);
  ...
  void
    InsertPoolChecks::registerStackObjects (Function &F) {
  ...
  DSNode* 
    InsertPoolChecks::getDSNode (const Value *VOrig, Function *F) {
#ifndef LLVA_KERNEL
      //
      // JTC:
  ...
  unsigned 
    InsertPoolChecks::getDSNodeOffset(const Value *V, Function *F) {
#ifndef LLVA_KERNEL
      DSGraph &TDG = paPass->getDSGraph(*F);
#else  
      DSGraph &TDG = TDPass->getDSGraph(*F);
